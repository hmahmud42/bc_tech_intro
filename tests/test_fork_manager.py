...
def test_fork_manager_update():
    ...
    for i in range(main_branch_len):
        ...
        fork_manager.update(fork_manager.get_longest_fork(), [block])
        base_trans [0] += 3
        base_trans [1] += 1
        prev_hash = block.block_header.block_hash
        block_list.append(block)
    
    for i, hash in enumerate(fork_manager.block_depth_map):
        print(f"{i}: {hash}")
